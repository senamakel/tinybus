...
pub trait Interface: Send + Sync + 'static {
    ...
    /// Whether `member` refuses an ordinary, monitor-visible call.
    ///
    /// Implementations generated by [`tinybus_macros::interface`] return
    /// `true` for methods annotated with `#[tinybus(confidential)]`. Dynamic
    /// interfaces remain ordinary by default.
...
impl<T: Interface + ?Sized> Interface for std::sync::Arc<T> {
    ...
    fn requires_confidential(&self, member: &MemberName) -> bool {
        (**self).requires_confidential(member)
    }
